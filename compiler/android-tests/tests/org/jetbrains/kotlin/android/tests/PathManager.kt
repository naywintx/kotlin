...
class PathManager(destinationDirectory: File, val testDataDirectories: List<File>) {
    ...
    val destinationSrcDirectory: File = destinationDirectory.resolve("src")

    fun getOutputForCompiledFiles(flavor: String): File = destinationTestClassesDirectory.resolve(flavor)
    ...
    fun clearDestinationFolder() {
        if (destinationTestClassesDirectory.isDirectory) {
            ...
            destinationTestClassesDirectory.mkdirs()
        }
        ...
        if (destinationSrcDirectory.isDirectory) {
            ...
            destinationSrcDirectory.mkdirs()
        }
    }
}
